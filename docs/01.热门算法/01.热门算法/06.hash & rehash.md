...
1. 当Hash表中的数据量不断增加时，如何避免哈希冲突带来的性能下降？
2. 为什么链式哈希可以有效解决哈希冲突，Redis是如何实现链式哈希的？
3. 当Hash表需要扩容时，为什么直接rehash会对系统性能造成巨大影响？
4. Redis采用了哪种策略来优化rehash的过程，避免阻塞主线程的正常操作？
5. 在渐进式rehash过程中，Redis是如何确保每次迁移数据的时间成本有限的？
6. 在高并发场景下，Redis如何通过调整rehash触发条件，来平衡性能与内存使用？
7. 当rehash正在进行时，如何保证新加入的键值对在新的Hash表结构中正确存储？
